...
pub trait Handler<T>: Clone + Send + Sized + 'static {
    type Future: Future<Output = Result<Response, RedisError>> + Send + 'static;
...
where
    ...
    Res: IntoResponse,
{
    type Future = Pin<Box<dyn Future<Output = Result<Response, RedisError>> + Send>>;
    ...
    ($($ty:ident),*) => {
            ...
            $( $ty: FromRequest + Send, )*
        {
            ...
            fn call(self, req: Request) -> Self::Future {
                Box::pin(async move {
                    $(
                        let $ty = match $ty::from_request(req.clone()).await {
                            Ok(value) => value,
